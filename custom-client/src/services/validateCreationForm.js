const validateCreationForm = (form) => {
  ...
  if(+form.released[0] >= 32 || isNaN(form.released[0]) ) errorToSet.released = true;
  if(+form.released[1] > 12 || isNaN(form.released[1]) ) errorToSet.released = true;
  if(+form.released[2] < 1950 && +form.released[2] !== 0 || isNaN(+form.released[2]) ) errorToSet.released = true;

  if(+form.rating > 5 || +form.rating < 0 || isNaN(form.rating)) errorToSet.rating = true;
